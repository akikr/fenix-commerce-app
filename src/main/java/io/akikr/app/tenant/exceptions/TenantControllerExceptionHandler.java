...
public class TenantControllerExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleGlobalException(Exception ex) {
        log.error("Error occurred at TenantController handleGlobalException, due to {}", ex.getMessage(), ex);
    ...
    public ResponseEntity<ErrorResponse> handleTenantException(TenantException ex) {
        log.error("Error occurred at TenantController handleTenantException, due to {}", ex.getMessage(), ex);
        String exMessage = ex.getMessage();
        String truncatedErrorMessage = exMessage.substring(0, Math.min(exMessage.length(), 80)) + "...";
