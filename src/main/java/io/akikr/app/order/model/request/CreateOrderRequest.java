...
public record CreateOrderRequest(
    ...
    @NotNull(message = "websiteId cannot be NULL") @NotBlank(message = "websiteId cannot be Blank") @JsonProperty("websiteId")
        String websiteId,
    @NotNull(message = "externalOrderId cannot be NULL") @NotBlank(message = "externalOrderId cannot be Blank") @JsonProperty("externalOrderId")
        String externalOrderId,
    @NotNull(message = "externalOrderNumber cannot be NULL") @NotBlank(message = "externalOrderNumber cannot be Blank") @JsonProperty("externalOrderNumber")
        String externalOrderNumber,
    @NotNull(message = "status cannot be null. Possible values: OPEN, COMPLETED, CANCELLED") @JsonProperty("status")
        OrderStatus status,
    @NotNull(
            message =
                "financialStatus cannot be null. Possible values: PENDING, PAID, REFUNDED, VOIDED")
    ...
    @NotNull(message = "orderTotal cannot be NULL") @JsonProperty("orderTotal")
        BigDecimal orderTotal,
    @Pattern(
            regexp = "^[A-Z]{3}$",
            message = "Currency must be in ISO 4217 format (e.g., USD, EUR)")
