...
public class OrderCommandServiceImpl implements OrderCommandService {
    ...
    public ResponseEntity<OrderUpsertResponse> upsertOrder(OrderUpsertRequest orderUpsertRequest)
        ...
        try {
            var store =
                    storeService.verifyStoreBelongsToTenant(orderUpsertRequest.orgId(), orderUpsertRequest.websiteId());
            if (Objects.isNull(store)) {
                throw new RuntimeException("Store with id:[ "
            ...
            throw new OrderException(
                    HttpStatus.BAD_REQUEST.value(), e, "An errorDetails occurred while Upserting order", "/orders");
        }
    }
    ...
    private static Order toOrder(OrderUpsertRequest orderUpsertRequest, Order existingOrder) {
        ...
        existingOrder.setOrderStatus(
                OrderStatus.valueOf(orderUpsertRequest.status().name()));
    ...
    private static Order buildOrder(Store store, OrderUpsertRequest orderUpsertRequest) {
        return Order.builder()
                ...
                .financialStatus(FinancialStatus.valueOf(
                        orderUpsertRequest.financialStatus().name()))
                .fulfillmentStatus(FulfillmentStatus.valueOf(
                        orderUpsertRequest.fulfillmentStatus().name()))
