...
public class StoreSpecifications {
    ...
    public static Specification<Store> withOptionalFilters(
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            if (isNotBlank(fromDate) && isNotBlank(toDate)) {
                ...
                predicate =
                        criteriaBuilder.and(predicate, criteriaBuilder.lessThanOrEqualTo(root.get("createdAt"), to));
            }
            if (Objects.nonNull(status)) {
                var storeStatus = Store.Status.valueOf(status.name());
                predicate = criteriaBuilder.and(predicate, criteriaBuilder.equal(root.get("status"), storeStatus));
            }
            if (Objects.nonNull(platform)) {
                var storePlatform = Store.Platform.valueOf(platform.name());
                predicate = criteriaBuilder.and(predicate, criteriaBuilder.equal(root.get("platform"), storePlatform));
            }
            if (isNotBlank(code)) {
                predicate =
                        criteriaBuilder.and(predicate, criteriaBuilder.like(root.get("storeCode"), "%" + code + "%"));
            }
            if (isNotBlank(domain)) {
                predicate =
                        criteriaBuilder.and(predicate, criteriaBuilder.like(root.get("domain"), "%" + domain + "%"));
            }
    ...
    public static Specification<Store> withSearchFilters(UUID tenantId, String websiteId, String code, String domain)
            throws RuntimeException {
        return (root, query, criteriaBuilder) -> {
            var predicate = criteriaBuilder.conjunction();
            if (Objects.nonNull(tenantId)) {
